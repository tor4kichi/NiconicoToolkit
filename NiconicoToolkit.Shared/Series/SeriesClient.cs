...
[JsonSerializable(typeof(NvapiSeriesVidoesResponseContainer))]
public sealed partial class SeriesJsonSourceGenerationContext : JsonSerializerContext
{
}
...
public sealed class SeriesClient
{
    ...
    public SeriesClient(NiconicoContext context, JsonSerializerOptions options)
    {
        _context = context;
        _options = new(options)
        {
            TypeInfoResolverChain = { SeriesJsonSourceGenerationContext.Default }
        };
    }
    ...
    public async Task<NvapiSeriesVidoesResponseContainer> GetUserSeriesVideosAsync(string seriesId, int page = 0, int pageSize = 100)
    {
        var uri = $"{NiconicoUrls.NvApiV1Url}series/{seriesId}?page={page + 1}&pageSize={pageSize}";
        return await _context.GetJsonAsAsync<NvapiSeriesVidoesResponseContainer>(uri, _options);
    }
}
